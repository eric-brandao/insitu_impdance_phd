function Z_qterm_quad=Z_qtermp(f,c0,hs,z,r,Z0,Vp)
...
% Z0 --> Fist estimative of surface impedance; vector of the same length
%         of f; 
% Vp --> Reflection coefficient obttained with 1p method (e.g 1 cm above the
...
Range(f(Range)<49)=[];     % Frequencies below 100 Hz are not calculated
...
Zf=Z0;         % First Guess - It comes from a simpler model of the sound field
...
%                     (sqrt((r^2)+(hs+z-1i*q).^2))));
                ...
                % iteration
                Vpc(b)=((exp(-1i*k*R2)/R2)-(2*k/Zg)*(quad(intp,0,30)))./(exp(-1i*k*R1)/R1);
%                 Vpc(b)=((exp(-1i*k*R2)/R2)-(2*k*Zg)*(quad(intp,0,20)))./(exp(-1i*k*R1)/R1);
